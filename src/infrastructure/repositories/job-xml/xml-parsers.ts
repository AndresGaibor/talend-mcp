import { asArray, parseXml } from "../../xml/xml-utils";
import type { TalendComponent, TalendConnection, TalendContextParameter, MapperEntry } from "../../../domain/job/job.entity";
import type { ParsedJob } from "../../../domain/job/job.repository";

type XmlRecord = Record<string, unknown>;

export function attr(nodo: XmlRecord | undefined, nombre: string): string | undefined {
  const valor = nodo?.[`@_${nombre}`];
  return typeof valor === "string" ? valor : undefined;
}

export function nodeAttributes(node: XmlRecord): Record<string, string> {
...
export function parseMapperEntries(node: XmlRecord): MapperEntry[] {
...
export function parseXmlToJob(xml: string, itemPath: string): ParsedJob {
