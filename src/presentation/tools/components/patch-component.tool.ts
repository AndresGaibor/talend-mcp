import { patchTalendComponentXml } from "../../../talend/editor";
...
type PatchComponentOptions = {
  uniqueName: string;
  patch: Record<string, string>;
};

export function createPatchComponentTool() {
  return {
    name: "patch_talend_component",
    description: "Aplica un parche a múltiples parámetros de un componente Talend.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        patch: {
          ...
          description: "Objeto con pares parámetro=valor a aplicar",
        },
      },
      required: ["jobName", "uniqueName", "patch"],
    },
    handler: wrapHandler("patch_talend_component", async (input: PatchComponentOptions & { jobName: string; folderPath?: string }) => {
      ...
      try {
        ...
        const updated = patchTalendComponentXml(xml, input.uniqueName, input.patch);
        await writeTextFile(job.itemPath, updated, projectPath);
        return ok({ itemPath: job.itemPath, uniqueName: input.uniqueName, patchedCount: Object.keys(input.patch).length }, { startTime: start });
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        if (msg.includes("no encontrado")) return fail("COMPONENT_NOT_FOUND", msg, { startTime: start });
        return fail("PATCH_COMPONENT_ERROR", msg, { startTime: start });
      }
    }),
  };
}