import { updateTalendSchemaColumnXml } from "../../../talend/editor";
...
export function createUpdateSchemaTool() {
  return {
    name: "update_talend_schema_column",
    description: "Actualiza una columna de schema de un componente Talend.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    handler: wrapHandler("update_talend_schema_column", async (input: SchemaColumnEditOptions & { jobName: string; folderPath?: string }) => {
      ...
      try {
        ...
        const updated = updateTalendSchemaColumnXml(xml, { uniqueName: input.uniqueName, schemaName: input.schemaName, columnName: input.columnName, patch: input.patch });
        await writeTextFile(job.itemPath, updated, projectPath);
        return ok({ itemPath: job.itemPath, uniqueName: input.uniqueName, schemaName: input.schemaName, columnName: input.columnName }, { startTime: start });
      } catch (err) {
        ...
        return fail("UPDATE_SCHEMA_ERROR", msg, { startTime: start });
      }
    }),
  };
}